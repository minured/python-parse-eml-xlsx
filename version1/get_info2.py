...
head = ["poid", "任务id", "达人主体", "达人昵称"]
save_ws.append(head)



#根据行和列获取值
def get_value(ws, row, renwuid_c, nicheng_c, zhuti_c, save_ws, poid):
##    print("向下查找值")
    n = 1
...
##        print(renwuid, nicheng, zhuti)
        ...
        save_ws.append([poid, renwuid, zhuti, nicheng])
...
##    print("    开始遍历当前行的列，确定其他字段")
    for col_i in range(ws.ncols):
        ...
        if temp_value == "达人昵称":
            ...
            nicheng_c = col_i
        elif temp_value == "达人主体":
            #print("        达人主体 %d,%d"%(row, col_i))
            zhuti_c = col_i

    return (nicheng_c, zhuti_c)
...
for index, fname in enumerate(file_names):
    ...
    for ws_i in range(len(ws_names)):
        ...
        try:
            ...
            nicheng_c, zhuti_c = get_other_col(ws, row)
            #print(nicheng_c, zhuti_c)

            get_value(ws, row, renwuid_c, nicheng_c, zhuti_c, save_ws, poid)
...
save_wb.save(os.path.join(desktop, "fill_v2.xlsx"))
